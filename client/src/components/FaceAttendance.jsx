        ...
        const screenshot_mechanism = setInterval(async ()=>{
            ...
            for (let i = 0; i < faceCanvases.length; i++) {
                ...
                try {
                    ...
                    context.beginPath();
                    context.lineWidth = "3";
                    context.strokeStyle = (res.data["confidence"] > 0.9) ? "lime" : "red";
                    context.rect(box.x, box.y, box.width, box.height);
                    context.stroke();
                    context.font = "20px courier";
                    context.fillStyle = (res.data["confidence"] > 0.9) ? 'lime' : 'red';
                    context.fillText(`${res.data["prediction"]} (${Number.parseFloat(res.data["confidence"]*100).toFixed(2)})`, box.x, box.y-25);

                    if (res.data["confidence"] > 0.9){
    ...
    return (
        ...
        <div className="w-full max-w-7xl bg-[#0d0d1f]/80 backdrop-blur-md py-2 px-8 rounded-3xl shadow-[0_8px_30px_rgba(255,0,255,0.3)] border border-fuchsia-800/30 flex gap-8">
            ...
            <div className="flex flex-col items-center flex-1">
                ...
                <div className="relative w-[680px] h-[440px] rounded-2xl overflow-hidden border border-cyan-400/40 shadow-inner shadow-cyan-500/30">
                    <Webcam
                        ref={webcamRef}
                        audio={false}
                        width={680}
